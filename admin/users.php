...
<?php 
    ...
    if (isset($_GET['delete'])) {
        ...
        $query0 = "SELECT role FROM user WHERE u_id = '$the_user_id'";
        ...
        $query = "DELETE FROM user WHERE u_id = '$the_user_id'";
    ...
    if (isset($_GET['change_to_admin'])) {
        ...
        $query0 = "SELECT role FROM users WHERE u_id = '$the_user_id'";
        $result = mysqli_query($conn , $query0) or die(mysqli_error($conn));
        if (mysqli_num_rows($result) > 0 ) {
            $row = mysqli_fetch_array($result);
            $id1 = $row['role'];
        ...
        $query = "UPDATE user SET role = 'admin' WHERE u_id = '$the_user_id'";
